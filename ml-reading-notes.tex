...
\section{Kinetics~\cite{kay2017kinetics}}
\label{kinetics}
...
\subsection{Revisiting the Effectiveness of Off-the-shelf Temporal Modeling
            ...
            Classification\cite{2017arXiv170803805B}}
...
following~\cite{DBLP:journals/corr/HersheyCEGJMPPS16}. Audio data is first
...
following~\cite{DBLP:journals/corr/WangXW0LTG16}.
...
\subsection{Designing Neural Network Architectures using Reinforcement
            Learning~\cite{DBLP:journals/corr/BakerGNR16}}

Q-Learning~\cite{Watkins1992} is used to build neural network architectures for
...
\subsection{Asynchronous Methods for Deep Reinforcement
            Learning~\cite{DBLP:journals/corr/MnihBMGLHSK16}}
...
of~\cite{DBLP:journals/corr/MnihBMGLHSK16}) involves using the value function
as a learned baseline in policy gradient. Hence the value-function baseline is
the ``critic'', while the policy is the ``actor''.

Entropy is added to the policy $\pi$ (\cite{williams1991function}) to avoid
...
approximation~\cite{DBLP:journals/corr/SeijenMPMS15}.
...
\subsection{Scalable trust-region method for deep reinforcement learning using
            Kronecker-factored
            approximation~\cite{DBLP:journals/corr/abs-1708-05144}}
...
approximating $F$ is the same as in~\cite{DBLP:journals/corr/MartensG15}.
...
\subsection{Skip-Thought Vectors~\cite{DBLP:journals/corr/KirosZSZTUF15}}
...
\subsection{Effective Approaches to Attention-based Neural Machine
            Translation~\cite{DBLP:journals/corr/LuongPM15}}
...
\subsection{Reinforcement Learning for Bandit Neural Machine Translation with
            Simulated Human Feedback~\cite{DBLP:journals/corr/NguyenDB17}}
...
\subsection{Recurrent Batch
            Normalization~\cite{DBLP:journals/corr/CooijmansBLC16}}
...
$\mathbf{h}_t = \sigma{(\mathbf{o}_t)} \odot \textrm{tanh}{(\mathbf{c}_t)}$,
