...
\subsection{Machine Learning: A Probabilistic Perspective}
...
\subsubsection{Generative Models for Discrete Data}
...
\subsection{Deep Learning~\cite{Goodfellow-et-al-2016-Book}}
...
\subsection{THUMOS~2014~\cite{DBLP:journals/corr/IdreesZJGLSS16}}
\label{thumos}
...
Post-processing is done to remove segments predicted as background, scale up
confidence scores by the class distribution in the training set. Non-maximum
suppression is done to remove redundant detections, with overlap threshold
$\theta - 0.1$, where $\theta$ is the IoU overlap threshold.
