...
\begin{equation}
        h^{(i)} = \textrm{max}\left(w^{(i)T}x, 0.01w^{(i)T}x\right) =
        \begin{cases}
...
The 70x70 discriminator architecture is $C64-C128-C256-C512$, 1x1 and 16x16 are
$C64-C128$, and 256x256 is $C64-C128-C256-C512-C512-C512$.
...
Random jitter was applied by resizing the 256x256 input to 286x286 and random
cropping back down to 256x256.
...
generated image at 256x256 resolution for one second, after which the
...
its accuracy on the 70x70 PatchGAN on cityscapes hit 0.63 per-pixel, 0.21 per
class and 0.16 class IoU, higher than 1x1 and 256x256 PatchGAN discriminators.
...
A generator is trained on 256x256 images, and evaluated on 512x512 map
$\leftrightarrow$ aerial images.
