...
\url{https://arxiv.org/abs/1803.09374}
}
...
superior performance in terms of Open-Ended accuracy on the VQA task. In
...
\headerbox{2.~VQA Task}{name=vqa-task,column=0,below=introduction,span=2}{%
We define a fusion operator for the VQA task as a function $\fusop$,
parametrized by~$\thetavec$, of the text-based question feature vector~$\q$ and
...
\headerbox{3.~Tucker Decomposition}{name=tucker-decomp,column=2,below=introduction,span=1}{%
...
\headerbox{4.~Comparison with MUTAN}{name=comparison-mutan,column=0,below=vqa-task,span=2}{%
Shown below we compare the MUTAN fusion operator of~\cite{ben2017mutan} with
...
\begin{center}
\hspace{-7.4cm}\resizebox{1.48\linewidth}{!}{\input{data/mutan-ours.tikz}}
\end{center}
...
and~$N_r\mutanfeat{v}$, followed by a neural network, producing a set of~$R$
...
\headerbox{5.~Ablation Study}{name=ablation,span=1,column=2,below=tucker-decomp}{%
We compare improvements in accuracy between different instantiations of the
...
\begin{centering}
\begin{tabular}{rc}
\multirow{2}{*}{\textbf{Model}} & \textbf{VQA 1.0} \\
& \textbf{val.\ acc.}\\
\midrule
...
\headerbox{6.~Results}{name=results,span=2,column=0,below=comparison-mutan}{%
We evaluate our best model on VQA~2.0 test-dev, and compare to previous
state-of-the-art models upon which our work is based.
...
\begin{tabular}{l*{5}{c}}
\centering
& \multicolumn{4}{c}{\textbf{VQA~2.0 test-dev accuracy}} \\
...
\headerbox{7.~Conclusions}{name=conclusion,column=0,below=results,span=2,above=bottom}{%
\begin{itemize}
        \item We generalized the MUTAN multimodal fusion operator.
...
\headerbox{8.~References}{name=references,column=2,span=1,below=ablation}{%
...
\headerbox{9.~Acknowledgements}{name=acknowledgements,column=2,span=1,below=references,above=bottom}{
